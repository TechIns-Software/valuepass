!(function (i) {
  ...
  i(window).on("load", function () {
    i('[data-loader="circle-side"]').fadeOut(),
      i("#preloader").delay(350).fadeOut("slow"),
      i("body").delay(350),
      i(".hero_in h1,.hero_in form").addClass("animated"),
      i(".hero_single, .hero_in").addClass("start_bg_zoom"),
      i(window).scroll();
  }),
    i(window).on("scroll", function () {
      1 < i(this).scrollTop()
        ? i(".header").addClass("sticky")
        : i(".header").removeClass("sticky");
    }),
    i("#sidebar").theiaStickySidebar({ additionalMarginTop: 150 }),
    i(".fixed_title").theiaStickySidebar({ additionalMarginTop: 180 });
  ...
  t.on("click", function () {
    ...
    i(".video").magnificPopup({ type: "iframe" }),
    ...
    i(".custom-search-input-2 select, .custom-select-form select").niceSelect(),
    ...
    i(window).bind("load resize", function () {
      i(window).width();
      i(this).width() < 991
        ? i(".collapse#collapseFilters").removeClass("show")
        : i(".collapse#collapseFilters").addClass("show");
    }),
    i(window).on("scroll", function () {
    ...
    i(".aside-panel-bt").on("click", function () {
      i("#panel_dates").toggleClass("show"),
        i(".layer").toggleClass("layer-is-visible");
    }),
    i(".content_more").hide(),
  ...
  n.find("a").on("click", function (e) {
    e.preventDefault();
    (e = this.hash), (e = i(e));
    i("html, body").animate({ scrollTop: e.offset().top - 140 }, 400, "swing");
  }),
    ...
    i('#faq_box a[href^="#"]').on("click", function () {
      if (
        ...
        var e = i(this.hash);
        if ((e = e.length ? e : i("[name=" + this.hash.slice(1) + "]")).length)
          return (
            i("html,body").animate({ scrollTop: e.offset().top - 185 }, 400), !1
          );
      }
    }),
    i("ul#cat_nav li a").on("click", function () {
      i("ul#cat_nav li a.active").removeClass("active"),
        i(this).addClass("active");
    }),
    ...
    i(".panel-dropdown a").on("click", function (e) {
      i(this).parent().is(".active")
        ? a()
        : (a(), i(this).parent().addClass("active")),
        e.preventDefault();
    });
  var s = !1;
  i(".panel-dropdown").hover(
    function () {
      s = !0;
    },
    function () {
    ...
    i(function () {
      i(window)
        .bind("resize", function () {
          i(this).width() < 768
            ? i(".input-dates").removeClass("scroll-fix")
            : i(".input-dates").addClass("scroll-fix");
        })
        .trigger("resize");
    }),
    i('a[href^="#"].btn_explore').on("click", function (e) {
      ...
      i("html, body")
        .stop()
        .animate({ scrollTop: e.offset().top - 50 }, 300, "swing", function () {
          window.location.hash = t;
        });
    }),
    i(".main-menu > ul > li").hover(
      function () {
        i(this).siblings().stop().fadeTo(300, 0.6),
          i(this).parent().siblings().stop().fadeTo(300, 0.3);
      },
      function () {
        i(this).siblings().stop().fadeTo(300, 1),
          i(this).parent().siblings().stop().fadeTo(300, 1);
      }
    );
})(window.jQuery);
